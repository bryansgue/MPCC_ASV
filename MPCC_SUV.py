...
def f_system_model():
    ...
    # --- Parámetros físicos (ejemplo, ajusta a tu vehículo real) ---
    m = 50.0
    Iz = 10.0
    Xu_dot = -5.0
    Yv_dot = -20.0
    Nr_dot = -5.0
    du, dv, dr = 2.0, 5.0, 1.0
    ...
    v     = MX.sym('v')   # sway
    r     = MX.sym('r')   # yaw rate
    x = vertcat(x_pos, y_pos, psi, u, v, r)
    ...
    v_p   = MX.sym('v_p')
    r_p   = MX.sym('r_p')
    xdot = vertcat(x_p, y_p, psi_p, u_p, v_p, r_p)

    # --- Controles ---
    tau_u = MX.sym('tau_u')  # fuerza en surge
    tau_v = MX.sym('tau_v')  # fuerza en sway (en tu caso real = 0)
    tau_r = MX.sym('tau_r')  # torque en yaw
    u_control = vertcat(tau_u, tau_v, tau_r)
    ...
    tau_v_d = MX.sym('tau_v_d')
    tau_r_d = MX.sym('tau_r_d')

    # Errores obligatorios
    el_x   = MX.sym('el_x')   # error longitudinal X
    el_y   = MX.sym('el_y')   # error longitudinal Y
    ec_x   = MX.sym('ec_x')   # error de contorno X
    ec_y   = MX.sym('ec_y')   # error de contorno Y
    theta_p = MX.sym('theta_p') # ángulo tangente de la referencia

    # Vector de parámetros externos
    p = vertcat(
        x_d, y_d, psi_d, u_d, v_d, r_d,
        tau_u_d, tau_v_d, tau_r_d,
    ...
    dv_dt = (1/(m - Yv_dot)) * (tau_v - dv*v - m*u*r)
    dr_dt = (1/(Iz - Nr_dot)) * (tau_r - dr*r)

    f_expl = vertcat(dx, dy, dpsi, du_dt, dv_dt, dr_dt)
    f_impl = xdot - f_expl
...
def create_ocp_solver_description(x0, N_horizon, t_horizon, bounded) -> AcadosOcp:
    ...
    R_u = 1 * np.diag([0.0001, 0.0001, 0.0001])
    ...
    error_actitud = theta_d - model.x[2]
    ...
    ## ERROR DE VELOCIDAD
    dx = model.x[3]*cos(model.x[2]) - model.x[4]*sin(model.x[2])  # u cosψ - v sinψ
    dy = model.x[3]*sin(model.x[2]) + model.x[4]*cos(model.x[2])  # u sinψ + v cosψ
    vel_inertial = vertcat(dx, dy)

    vel_progres = dot(tangent_normalized, vel_inertial)


    # Define el costo externo considerando los errores como vectores
    error_cost = 1*error_actitud**2 
...
def main():
    ...
    for k in range(0, t.shape[0]-N_prediction):
        ...
        for j in range(N_prediction):
            ...
            yref = xref[:,k+j]
            parameters = np.hstack([yref, psid[k+j] , 0, 0, 0, 0, 0])
            acados_ocp_solver.set(j, "p", parameters)
        
        yref_N = xref[:,k+N_prediction]
        parameters_N = np.hstack([yref_N, psid[k+N_prediction], 0, 0, 0, 0, 0])
    ...
    fig3 = animate_SUV_pista(x[:3, :], xref[:2, :], left_poses[:, : , :], rigth_poses[:, :, :], 'animation_MPCC_SUV.mp4')
